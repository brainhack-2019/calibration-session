...
info_1 = visual.TextStim(win=win, name='info_1',
    text='Make the gesture written on the screen.\n\nDo it once the square appears.\n\n(This is a training session)',
    font='Arial',
...
instructionComponents = [info_1, key_resp_2]
...
while continueRoutine:
    ...
    # *info_1* updates
    if t >= 0.0 and info_1.status == NOT_STARTED:
        # keep track of start time/frame for later
        info_1.tStart = t
        info_1.frameNStart = frameN  # exact frame index
        info_1.setAutoDraw(True)
...
for thisTrial in trials:
    ...
    text.setText(name)
    # keep track of which components have finished
    trialComponents = [text, polygon, diode]
    ...
    while continueRoutine and routineTimer.getTime() > 0:
        ...
        # *polygon* updates
        if t >= 2 and polygon.status == NOT_STARTED:
            # keep track of start time/frame for later
            polygon.tStart = t
        ...
        # *diode* updates
        if t >= 2 and diode.status == NOT_STARTED:
            # keep track of start time/frame for later
            diode.tStart = t
            diode.frameNStart = frameN  # exact frame index
            diode.setAutoDraw(True)
        frameRemains = 2 + 2- win.monitorFramePeriod * 0.75  # most of one frame period left
        if diode.status == STARTED and t >= frameRemains:
            diode.setAutoDraw(False)
...
# keep track of which components have finished
trainingComponents = [text_2, key_resp_3]
